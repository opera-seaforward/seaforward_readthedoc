...
!!! important
    **Why these values.** `dt=300` s and `NTIMES=2016` run 7 days (2 spin-up + 5 forecast) as one continuous simulation. `9999 1 24 9999 1` is the dummy-date convention that pairs with the `Y9999M01` forcing files (24 = hourly records). With `USE_CALENDAR` off (the regional default), CROCO ignores real calendar dates and just steps through the records — so you don't hand-edit real dates here.
...
!!! important
    **If it crashes with `Abnormal termination: netCDF INPUT`** right after "Open Meteo file" — it's the GFS longitude issue; redo Step 6.**If numbers go `NaN` / it says `BLOW UP`** — an instability: recheck the open boundaries (Step 3) match the mask, and that the timestep isn't too large.