...
**What this does:** reads the sea-floor depth data (ETOPO2), works out which grid points
are land vs ocean (the "mask") from the coastline, and smooths the bathymetry so the
model stays stable. The scrolling `rx0`/`ry0` numbers are the smoothing working; they
settle near `0.20`. It finishes with `Writing .../CROCO_FILES/croco_grd.nc done`.
...
<figure style="text-align: center; margin: 20px 0;">
  ...
  <figcaption style="font-size: 1em; color: #555; margin-top: 8px; font-style: italic;">
    Building the model grid: your domain limits and grid spacing pass through
    <code>grid.ini</code> to the CROCO pytools, which combine them with the GSHHS
    coastline and ETOPO2 bathymetry. The result feeds every later step.
  </figcaption>
</figure>
...
This one is Python, not a shell command, and it runs from `~/seaforward` — the blocks
above were in the prepro directory, so change back first:
...
The left panel is the grid mesh, drawn over ocean cells only — the lines stop at the
coast, so the mesh shows the computational domain. The right panel is the smoothed
bathymetry the model will use. Check the shelf break looks continuous and that nothing is
