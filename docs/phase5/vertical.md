...
Two ways to look below the surface: a **section**, which slices the ocean along a line and
shows the water column across it, and a **profile**, which takes a single point from
...
The simplest way below the surface is the same map you drew in Surface fields, with a
depth. The difference between the surface and 100 m is often striking — the surface
...
Points where the sea floor is shallower than the requested depth come back blank — the
physically correct "no water here".
...
coordinate — so it plots against the real bathymetry, with land and coastal edge cells
left blank.
...
pp.section(ds, 'temp', -21.0, 21.0, -17.2, 21.0)
```
...
This is where stratification becomes visible: the thermocline's depth, how it tilts across
the shelf, and the cold water drawn up at the coast.
...
Where the jets sit in the vertical — surface-intensified flow, or a subsurface core over
the slope.
...
magnitude — which matters on a coast like this one, where the along-shore and cross-shore
components behave quite differently.
...
Sections work for raw fields (`temp`, `salt`, `u`, `v`) and derived ones (`speed`, `ke`,
`vort`, `vort_f`). SSH is two-dimensional and has no section.
...
The shape tells you the structure: a near-uniform layer at the top is the mixed layer, the
sharp gradient below it the thermocline, and the slow decrease beneath that the deep water.

Two things worth doing on every profile. **Crop the depth range** — over 4000 m of water
the interesting structure is in the top kilometre and the rest is a flat line. And **set
the title** — without it the plotter stamps the coordinates into the title, which on a
narrow figure runs wider than the plot itself.
...
pl.plot(pp.field(ds, 'temp', depth_m=100), ds=ds, isobaths=iso,
        out='g_temp100.png')
...
for var, name in fields:
    pl.plot(pp.section(ds, var, lon0, lat0, lon1, lat1),
            title=name + ' — section', out='g_sec_' + var + '.png')
