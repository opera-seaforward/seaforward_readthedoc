...
Everything so far has been a single moment. These views add the time axis: a **Hovmöller**
diagram collapses one spatial dimension against time in a single panel, a **time series**
follows one point through the run, and an **animation** plays the whole field forward.

The point is the same one the profiles use, so the pages describe the same place from
different angles.

Start a Python session and set up:
...
The snippets below continue in that session. The last section repeats everything as one
...
- **`kind='time_lat'` or `'time_lon'`** — a transect against time. Structures that
  propagate — eddies, waves, fronts — appear as tilted bands, and the slope of the tilt is
  their speed.

The signatures, and one filled in:
...
The water column at one point, through the run. Crop the depth range or most of the panel
is uniform deep water:
...
A meridional transect against time — this is where propagating features show themselves,
as bands that tilt rather than run flat.
...
One point, one variable, through the whole run. This is where diurnal cycles, transient
events and any drift show up.

The signature, and an example:

```python
pp.timeseries(ds, var, lon0, lat0, surface_only=True, depth_m=None)
pp.timeseries(ds, 'temp', -19.0, 21.0, depth_m=50)
```
...
Passing `depth_m` follows the same point below the surface — useful for separating what the
atmosphere is doing from what the ocean is doing.
...
The most direct way to see time: play the field forward. `sftools.animation` reuses the
same colour scales and overlays as the static plots, and resolves the limits once across
the whole series so the scale doesn't flicker between frames.
...
Without `out=`, it returns a widget that plays inline in Jupyter. With `out=`, it writes a
file — `.gif` through pillow, anything else through ffmpeg:
...
Currents at depth are weaker than at the surface, so `scale=2` lengthens the arrows — the
parameter is inverse, and the default of 8 leaves them almost invisible here.

### Current speed at depth

`depth_m` sets the depth of the **shaded** field, `uv_depth` the depth of the **arrows**.
Set both and the whole figure sits at one level:
...
Leave `depth_m` out and the shading stays at the surface — which is worth knowing, because
a surface-shaded field with 200 m arrows over it is misleading rather than informative.
...
or the surface), or `None`. `isobaths=[200, 1000]` adds bathymetry contours, `scale` sets
arrow length, `skip` thins them, `tindex_range=(0, 20)` limits the frames, `interval` sets
the inline frame delay and `fps` the written one.

## Reproducing these figures

Everything above, as one runnable block. The animations take minutes rather than seconds:
...
for var, name in [('temp', 'Temperature at 50 m'), ('salt', 'Salinity at 50 m'),
    ...
    pl.plot(pp.timeseries(ds, var, lon0=plon, lat0=plat, depth_m=50), title=name,
            out='g_ts_' + var + '50.png')

# the two animations at depth interpolate the whole grid on every frame
anim.animate(ds, 'temp', overlay='wind', vmin=20, vmax=26, out='anim_1.gif')

anim.animate(ds, 'zeta', overlay='uv', uv_depth=200, scale=2, skip=3,
             vmin=-0.25, vmax=-0.05, cmap='Spectral_r', out='anim_2.gif')
