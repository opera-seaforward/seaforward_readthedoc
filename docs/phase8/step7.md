...
they are filtered by the parent's own `OBC_*` settings, and Canary's east boundary is
closed on the African coast.
...
```text
 CANARY_12 FORECAST                     <- parent (croco.in)
    ...
    300.00  dt
...
 CANARY_12 AGRIF ZOOM LEVEL 1           <- child (croco.in.1)
       288  ntimes
...
```text
      ...
      38  9686.04398 1.754789696E-03 3.5356319E+01 3.5358074E+01 6.6786856E+14  0
      13  9686.04514 1.530197898E-03 3.8066552E+01 3.8068082E+01 1.8319676E+15  0   <- parent
      39  9686.04514 1.755278409E-03 3.5356337E+01 3.5358092E+01 6.6786837E+14  0
```
...
parent advances slowly (12, 13, …) with `NET_VOLUME ≈ 1.83e+15`; the child advances
three times faster (36, 37, 38, 39, …) with `NET_VOLUME ≈ 6.68e+14`.

**Three child steps per parent step, meeting at identical times.** Parent 12 and child
...
B = 'forecast/scratch/Canary_AGRIF/CROCO_FILES/'
...
vmin, vmax = 18, 28                       # shared scale, so the panels compare
...
ax[1].set_title('AGRIF child  1/36 (2.88 km)')
for a in ax:
...
sharper edges. The anticyclonic spiral at 18°W, 19.5°N is a blocky curl at 9.2 km and
a coherent eddy at 2.88 km.