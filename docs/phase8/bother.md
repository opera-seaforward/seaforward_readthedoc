...
**Where does the correction apply?** Directly, only inside the child's footprint. But
the parent's own dynamics then carry those corrected values outward, so after a day
you see differences well beyond the box, following the flow. The parent does not gain
...
| Execution | sequential, 2 runs | simultaneous, 1 executable | simultaneous, 1 executable |
...
**The costs are real**, and they constrain what you can build: a fixed refinement
ratio, a shared vertical grid, and a feedback step that can destabilise if the grids
disagree at the interface. The next page sets out exactly what each one rules out.