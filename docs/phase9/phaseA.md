...
`EXTENTS` is all-positive (eastern hemisphere), so the `--domain=` quoting trouble that
bites western regions doesn't apply — and **Phase 2 Step 6 (the GFS longitude fix) gets
skipped entirely**. Same as IGOG.
...
!!! important
    **Read the file, don't trust the arithmetic.** 13° / (1/12°) + 2 predicts 158. The generator produced **159**. It rounds to whole cells, and it is the authority. This is the same lesson as the grid's actual north edge landing at **31.79°S** rather than the 32.0 requested.
...
Phase 2's rule of thumb ("mostly `.` → closed") says close it. **We opened it anyway.**
The reasoning:

- The 18 water cells are not noise. The **west group (16.6–17.7°E)** is the Benguela /
  South Atlantic; the **east group (30.0–30.4°E)** is where the **Agulhas Current
...
Contrast Canary, which closed its east edge at ~1/123 ocean — that was one cell of a
...
South African shelf and the current's inshore path. That's the domain, not a detail.
...
MERCATOR_20260717_00.nc            206 MB
```

> The bry is **larger than IGOG's 3.3 MB despite a smaller grid** — because it carries
> four boundaries instead of two. That's a free check that `obc_dict` took effect.
...
| boundaries | **nothing** — all four `OBC_*` stay `define` |
...
sed -n '185,192p' cppdefs.h        # ONLINE on, AROME + ERA_ECMWF off
```

!!! warning
    ⚠️ `grep -n "define ONLINE"` finds nothing even when it's correct — the file has `# define  ONLINE` with **two** spaces. Grep for `ONLINE` alone.
...
**`croco.in`** — title → `AGULHAS_12 FORECAST`; check S-coord is `7.0d0 2.0d0 200.0d0`
(must match `sigma_params`); sponge → `50000. 400.`
...
they're set per-run by the driver.
...
Current, and it's the correct answer. A quiet KE here would mean the current was
missing.

**`rx0 = 0.2001`** — exactly the `rfact = 0.2` requested. The smoother hit its target.
(An AGRIF child won't: `merging_area` blends the parent's rougher bathymetry back in at
the edges, so IGOG's child got 0.347 from the same 0.2.)

**`rx1 = 14.84`** — high. Above ~10 you watch it; above ~20 expect trouble. This is the
Agulhas shelf break: 100 m to 4000 m over a short distance, with 50 sigma layers
stretched across the tilt. **It ran stably** — KE steady, `trd = 0` throughout — so 14.84 is demonstrably survivable at 1/12°.

But it set up the open question for Phase B: **a 3× child resolves that same slope with
thinner layers, so its `rx1` will likely be worse.**

**And `dt = 300` survived 2 m/s.** That was the other worry — CFL scales with velocity,
and the Agulhas is fast. It didn't blink. Good news for the child, which gets `dt = 100`
and therefore even more headroom.
...
HIS = 'forecast/scratch/Agulhas_12/CROCO_FILES/croco_his.nc'
ds  = pp.open_history(HIS, Yorig=2000)
sst = pp.field_map(ds, 'temp', tindex=-1, level=-1)   # last record, surface sigma
pl.plot_map(sst, cmap='RdYlBu_r', out='docs/img/agulhas_12_sst.png')
"
```

![Agulhas_12 sea-surface temperature](../img/agulhas_12_sst.png)

*Surface temperature after the 1-day proof run.*

This one figure tells you the config is right, and it's worth reading in detail:

- **The Agulhas Current** — the deep red (22–23 °C) entering at 30°E against the coast,
  then running southwest along the shelf edge. Tropical Indian Ocean water riding down
  the African coast, exactly where it belongs.
- **The retroflection** — near 24–26°E the warm band leaves the shelf and turns west;
  the red tongue at 39°S running back east from 17–22°E is Agulhas Return Current
  water. The current overshoots and doubles back.
- **Benguela upwelling** — the dark blue (≈13 °C) pinned against the west coast at
  18°E. Cold deep water surfacing. Nothing in the config asked for it; it's there
  because the physics is right.
- **Eddies** — a cold core near 24°E, 38°S; cyclonic structure near 18°E, 37.5°S.

**It also vindicates the north-boundary decision** (A3). Look at 30°E, 33°S: the
current is entering through the **eastern corner of the north edge** — those 5 water
cells out of 159 that the rule of thumb said to close. Closed, the current would have
had nowhere to come from.

!!! note
    **The honest caveat.** This is one day from a Mercator cold start, so most of this structure *is* Mercator's, lightly adjusted. The model hasn't generated these features — it has inherited them and kept them stable. That is still exactly the result you want from a proof run: it means the interpolation, the boundaries, and the forcing all work together. Judging the model's own dynamics needs a spun-up run.

### Comparing against Mercator

The picture looks right, but "looks right" is not a check. Compare it against the
source directly.
...
for i, t in enumerate(d.time.values): print(' ', i, t)
import sftools.postprocess as pp
...
c = xr.open_dataset('forecast/scratch/Agulhas_12/CROCO_FILES/croco_his.nc', decode_times=False)
sst_c = c.temp.isel(time=-1, s_rho=-1).where(c.mask_rho == 1)
lon2d = c.lon_rho.values; lat2d = c.lat_rho.values

# --- MERCATOR: the matching date (idx 2), surface level ---
...
# regrid Mercator onto CROCO's curvilinear grid (pointwise interp)
...
lim = float(np.nanpercentile(np.abs(d), 99))     # 99th pct, not max: one bad cell
                                                 # would flatten the whole colour scale
...
for x in ax: x.set_xlabel('lon'); x.set_ylabel('lat')
...
- **The differences trace the fronts**, and they come as **blue/red dipoles** — a warm
  streak beside a cold streak along every frontal boundary. That is the signature of a
  front being **displaced by a cell or two**, not of a magnitude error. Both fields
  agree the front exists; they disagree slightly about exactly where. After 24 h with
  different numerics, that is the expected outcome.
- **±0.7 °C max, in thin filaments only.** No broad patches, no drift, no runaway.
...
| **broad patches** in a water mass | the model is drifting; check forcing |
...
There is faint banding along the **west edge (17–18°E)** here, which is worth watching
in a longer run. At one day it is most likely the boundary still adjusting.

!!! important
    **What this does and doesn't prove.** The model's IC *came from* Mercator at 07-15, so after 24 h they cannot have diverged much. This is a **consistency** check, not a skill test: it proves the interpolation didn't corrupt anything, the boundaries feed sensible water, and the forcing is being read. Real skill assessment needs a spun-up run compared against **independent** data — satellite SST, drifters, Argo — not against the model's own initial condition.
