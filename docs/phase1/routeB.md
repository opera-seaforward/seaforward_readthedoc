...
- `make -j ${NJOBS}` compiles using the processor count you set in
  [The two ways to build](step70.md). Confirm it's still set (`echo ${NJOBS}` — if
  blank, re-do the `export` there).
- `2>&1 | tee X.log` saves each step's output to a log you can inspect if
  something fails.

!!! warning
    **Dependencies for netcdf-c.** It needs `libcurl` and `m4` headers. You installed these in [System build tools](step1.md) (`libcurl4-openssl-dev m4`). If a configure step complains about curl or m4, install them and re-run that library.