...
- **Route B (by hand)** — type each library's `configure`, `make` and
  `make install` yourself. Slower, but you see exactly what happens; use it to
  learn, or if a script fails and you want to debug a single library.
